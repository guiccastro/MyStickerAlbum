...
class CreateEditAlbumViewModel @Inject constructor(
    ...
    init {
        _uiState.update {
            it.copy(
                ...
                deleteStickerDialog = DialogValues(changeDialogState = ::changeDeleteStickerDialogState),
                editStickerDialog = DialogValues(changeDialogState = ::changeEditStickerDialogState),
                currentStickersDialog = DialogValues(changeDialogState = ::changeCurrentStickersDialogState)
            )
        }
    ...
    private fun changeStickerDialog(sticker: Sticker) {
        _uiState.update {
            it.copy(
                ...
                stickerDialogIdTextField = it.stickerDialogIdTextField.copy(
    ...
    private fun onStickerIdChange(text: String) {
        val (error, errorMessage) = verifyStickerIdChange(text)
        _uiState.update {
            it.copy(
                stickerDialogIdTextField = it.stickerDialogIdTextField.copy(
                    text = text,
    ...
    private fun verifyStickerIdChange(text: String): Pair<Boolean, Int> {
        val stickerExists =
            _uiState.value.album.stickersList.stickers.find { it.identifier == text } != null
        return if (stickerExists) {
            Pair(true, R.string.error_sticker_id_exists)
        } else {
            Pair(false, R.string.error_sticker_id_exists)
        }
    }

    private fun saveIdSticker() {
        val newId = _uiState.value.stickerDialogIdTextField.text
        if (!verifyStickerIdChange(newId).first) {
    ...
    private fun changeDeleteStickerDialogState() {
        _uiState.update {
            it.copy(
                deleteStickerDialog = it.deleteStickerDialog.copy(showDialog = !it.deleteStickerDialog.showDialog)
            )
        }
    }

    private fun changeEditStickerDialogState() {
        _uiState.update {
            it.copy(
                editStickerDialog = it.editStickerDialog.copy(showDialog = !it.editStickerDialog.showDialog)
            )
        }
    }

    private fun changeCurrentStickersDialogState() {
        _uiState.update {
            it.copy(
                currentStickersDialog = it.currentStickersDialog.copy(showDialog = !it.currentStickersDialog.showDialog)
            )
        }
    }
}